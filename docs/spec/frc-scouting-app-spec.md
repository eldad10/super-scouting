...
**Status:** DRAFT v0.18 — topics 1–9 CLOSED; core architecture forks decided (storage, versioning, offline stats, stack, access model, language, roles/auth, data-entry UX, offline-first & sync, realtime deferred → 45s refresh). Remaining topics OPEN/PARTIAL pending their sub-questions.
...
All seven top blockers are now closed, and **topics 1–9 are fully CLOSED** (Q9.4 duplicate-scout resolution and Q7.5 offline conflict resolution both settled — one canonical entry per (team, match), last-write-wins per §7.3). Next-tier durable question still open: Q13.1 (coverage matrix + outlier flagging in v1).
