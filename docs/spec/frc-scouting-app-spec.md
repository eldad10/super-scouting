...
- **Configurable operational (meta) statistics (Q10.9-ops):** the general/operational statistics page is **built with the same builder, not static**, pointed at **app-metadata data sources** instead of form fields (§10.2). Exact meta-field list finalizes when Topic 13 closes.
...
**Two kinds of statistics (Q10.9-ops, closed 2026-08-21).** The app has (a) the **dynamic robot/team statistics** built by pointing charts at form fields, and (b) **operational (meta) statistics** — data *about the scouting itself*, not robot performance. **(b) is configurable, not static:** it uses the **same dashboard builder** with the **data source switched from form fields to app metadata.** Meta **dimensions**: scouter/user, match, event, form. Meta **metrics** (with count / sum / avg / % / min-max as available): entries submitted **per user / per match / per event / total**; **coverage** (matches scouted vs. scheduled, robots covered of 6 per match, teams scouted of total); qualification matches recorded vs. remaining; **missing / duplicate / conflicting** entry counts; **pending-sync (outbox) count and last-sync time**; active forms this season and **fields per form**; super-scouting coverage. This overlaps the coverage/quality matrix (Topic 13); the coverage matrix is one **prebuilt** operational view, and the **exact meta-field catalogue is finalized when Topic 13 closes** to avoid duplication.
...
All seven top blockers are now closed, and **topics 1–10 are fully CLOSED**. Next-tier durable question still open: Q13.1 (coverage matrix + outlier flagging in v1) — which also finalizes the operational-stats meta-field catalogue (§10.2).
