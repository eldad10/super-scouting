...
- **Season scoring model** (Q2.1, Q2.8). A field's raw value maps to game points (boolean → points if true; counter/number → points per unit; single/multi-select → points per selected option), plus a per-field success definition. Points are **entered inline in the form builder** but held in a **scoring model versioned independently of the form version**: entries store **raw observations only** and the score is **derived**, so **a scoring change never creates a new form version** and never invalidates collected data (Q2.8d). Editing a field carries its scoring config with it automatically — no separate manual step. Captured at field-creation time (same non-backfillable gotcha as semantic metadata, §3.3). Remaining mechanics (phase, negatives, totals) are Q2.8 → topics 3 & 9.
...
**Q2.8 — scoring model.** ✓ **Confirmed:** a field's raw value maps to points (boolean → points-if-true; counter/number → points-per-unit; single/multi-select → points-per-selected-option; plus a per-field success flag). Points are **entered inline in the form builder** but stored in a **scoring model versioned independently of the form**; entries hold raw observations only and the score is **derived**. Editing a field automatically updates its scoring entry (the model is keyed by field). Remaining sub-questions are spun out to **topics 3 (field config) & 9 (metric engine)** and do not block closure:
...
**Closed 2026-08-17:** Q2.1 – Q2.7 → confirmed requirements (**topic 2 CLOSED**); Q2.9 closed (own robot = regular robot); Q4.3 closed (flat events, no division hierarchy). Q2.8d closed (scoring change ≠ new form version). New spun-out: Q2.8a–c, Q2.8e, Q2.8f → topics 3 & 9.

- ~~**Topic 1:** Q1.1 – Q1.6~~ ✓ **CLOSED** — all confirmed in §1.1.
- ~~**Topic 2:** Q2.1 – Q2.7~~ ✓ **CLOSED** — confirmed in §2. Own robot = regular robot (Q2.9 closed). Spun out: Q2.8a–c, e, f (scoring mechanics) → topics 3 & 9.
- **Topic 3:** Q3.1 – Q3.10
...
All seven top blockers are now closed. Next-tier durable questions still open: Q3.7 (delete-form semantics), Q9.4 (duplicate-scout resolution), Q6.5 (dead-robot status), Q5.2 (scouter login), Q2.8a–c/e/f (scoring-model mechanics → topics 3/9).
