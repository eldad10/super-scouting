...
**Status:** DRAFT v0.26 — **topics 1–15 and 18 CLOSED.** Every feature topic plus the system architecture. Core forks decided (storage, versioning, offline stats, stack, access model, language, roles/auth, data-entry UX, offline-first & sync, realtime deferred → 45s refresh, dashboards & visualisation, search/ranking/browse, data quality, alliance selection, repo layout & services). Topic 14 is closed as **deferred in full to §24**. Remaining: 19, and the partials 16/17/20.
...
- **The API key is obtained when the feature is built** (Q14.2). A TBA read key is free from a TBA account; nothing needs requesting now. When requested it is a **team-owned** credential per §18.1 (Q18.5) and lives only in the server project's environment.
...
- ~~**Q14.2** — Do you have (or can you get) a TBA read API key?~~ ✓ **CLOSED 2026-09-01: obtained when the feature is built** — free, team-owned, server-side only (§14.2).
...
- **Three environments: production, preview and local.** Each Vercel project has its own root directory and build command, with **Turborepo build caching**. **Preview deployments (one per pull request) point at the *dev* Supabase project** — never at production; environment-variable sets are configured per Vercel environment so this cannot happen by accident. *(confirmed 2026-09-01)*
- **Migrations live in `packages/db`** and are applied via the **Supabase CLI**. The schema must be reproducible from the repo alone; a schema is never hand-edited in the Supabase dashboard, in either project. *(confirmed 2026-09-01)*
- **No custom domain in v1** (Q18.1). The app is reached at its **`*.vercel.app`** URL, and the client Vercel project is *named* so that URL is short and typeable on a phone in a pit; the exact URL is recorded in `docs/ops/SETUP.md`. Attaching a custom domain later is a dashboard change with **no code impact**, so nothing is designed around the hostname. *(decided by Claude 2026-09-01)*
- **Error tracking: Sentry free tier on both apps** (Q18.3). Uncaught errors and unhandled promise rejections are reported from client and server. Hard constraints: **no PII** — no names, usernames or free-text scouter notes in any event payload (scrubbed in `beforeSend`, consistent with the no-audit-log decision in Q5.5); **no performance tracing and no session replay** (free-tier quota and client bundle size); the SDK is **loaded lazily and must never block app boot or any offline path**; client errors captured while offline are queued and flushed on reconnect. Vercel's runtime logs remain the second server-side source. **Usage/product analytics are explicitly not wanted.** *(decided by Claude 2026-09-01)*
- **No self-hosted deployment and no venue laptop** (Q18.4). The offline-first PWA *is* the venue answer: full local operation, IndexedDB drafts and outbox, the cached active competition, offline statistics, and the QR fallback for moving entries without connectivity (§7). **The app never requires a server present at the venue**, and equally we will not build, document or support a self-hosted stack. Device-to-device local-network sync stays deferred in §24. *(decided by Claude 2026-09-01)*
- **All third-party accounts are team-owned, never personal** (Q18.5). A **team-owned email account** owns Vercel, Supabase and (when it exists) the TBA key; credentials live in the team's password manager with **at least two mentors/adults holding access**, so the platform cannot leave with a graduating student. `docs/ops/SETUP.md` carries the **account inventory** and an **annual handover checklist**: confirm both account holders, rotate the service-role and API keys, and wake and verify the dev and production Supabase projects before the season. *(decided by Claude 2026-09-01)*
- **A pre-event operational checklist** lives in `docs/ops/SETUP.md` (`COLLABORATION.md` §8 rules 5–7, §17.1): wake the production Supabase project, verify a full offline run on a real device with the network actually off, and take a **pre-event export** of the season. *(confirmed 2026-09-01)*

### 18.2 Closed questions

- ~~**Q18.1** — Do you have a **domain**, or is a `*.vercel.app` URL fine?~~ ✓ **CLOSED 2026-09-01: `*.vercel.app`, with a deliberately short project name; a custom domain is a later no-code change** (§18.1).
...
- ~~**Q18.3** **[RAISED BY ME]** — Do you want error/usage monitoring (Sentry, or Vercel's built-in analytics)?~~ ✓ **CLOSED 2026-09-01: Sentry free tier for errors on both apps — no PII, no tracing/replay, lazily loaded, offline-safe; no usage analytics** (§18.1).
- ~~**Q18.4** **[RAISED BY ME]** — Is there any scenario where you need this to run **fully self-hosted or on a laptop at the venue**?~~ ✓ **CLOSED 2026-09-01: no — the offline-first PWA is the venue answer; self-hosting is out of scope** (§18.1).
- ~~**Q18.5** **[RAISED BY ME]** — Who holds the Vercel and Supabase accounts?~~ ✓ **CLOSED 2026-09-01: team-owned accounts, two adults with password-manager access, account inventory + annual handover checklist in `docs/ops/SETUP.md`** (§18.1).
...
|---|---|---|---|
| 2026-09-01 | 18 | **Topic 18 CLOSED.** §18.2's proposals promoted into confirmed requirements: three environments (production / per-PR preview / local) with **preview pointing at the dev Supabase project**, Turborepo caching, migrations in `packages/db` applied by CLI only, and the client/server env-var split. Four questions decided by Claude: **no custom domain in v1** — a deliberately short `*.vercel.app` URL (Q18.1); **Sentry free tier for errors on both apps**, no PII, no tracing or replay, lazily loaded, offline-queued, and **no usage analytics** (Q18.3); **no self-hosted or venue-laptop deployment** (Q18.4); **all accounts team-owned**, two adults with password-manager access, plus an account inventory and annual handover checklist in `docs/ops/SETUP.md` (Q18.5). A pre-event operational checklist added to the same file. | The hostname is the one deployment decision with zero code coupling, so buying a domain now would be spending money to defer nothing — while a bad `*.vercel.app` name is the actual pit-day cost, hence naming the project deliberately. Error tracking earns a third-party account only for the case it uniquely covers: a client-side crash at a venue that nobody can reproduce afterwards; the constraints (no PII, no tracing, lazy load) exist so it can never contradict Q5.5's no-audit-log decision, the free-tier budget in §17.1, or the rule that nothing may block the offline path. Self-hosting was worth asking once and closing hard: §7 already delivers venue independence without a server, so a laptop deployment would add a second topology for a case that no longer exists. Team-owned accounts are the cheapest decision here and the one that has historically killed the most team tools — a graduating student holding the Supabase login is an outage scheduled on a calendar. |
| 2026-09-01 | 14 | **Topic 14 CLOSED — deferred in full to §24.** No external data source in v1. Q14.2–Q14.5 answered anyway and kept as §14.2's *recorded design* for when the import is built: **TBA read key obtained at build time**, team-owned, server-side only; **results imported, not just the schedule**; **manual "sync now" plus a throttled automatic sync when online**; and a **re-imported schedule reconciles rather than overwrites** — entries stay bound to their `(team, match)`, changed or vanished matches are flagged for review, surrogate matches are marked so the metric engine can exclude them, and a re-import is shown as a diff to confirm. The §24 row now points at §14.2. | Q14.1 already deferred the feature on 2026-08-14, but leaving the topic OPEN with four live questions made it read as unfinished v1 scope; closing it removes the ambiguity without discarding the thinking. Answering the four questions now is nearly free and prevents the worst version of this feature — a future session designing a blind overwrite that silently rewrites the `(team, match)` keys a weekend of scouting is bound to. The manual-entry path stays first-class precisely because v1 depends on it: built later as a fallback to the import, it would be built worse. |
...
**Closed 2026-09-01:** Q18.1, Q18.3–Q18.5 → confirmed requirements (**topic 18 CLOSED**), §18.2's proposals promoted. **Topic 14 CLOSED — deferred in full to §24**; Q14.2–Q14.5 answered and recorded as the deferred design (§14.2).
...
- ~~**Topic 14:** Q14.1 – Q14.5~~ ✓ **CLOSED 2026-09-01 — deferred in full to §24.** No external data source in v1. Q14.2–Q14.5 answered and kept as the recorded design in §14.2: key obtained at build time (team-owned, server-side only); results imported as well as the schedule; manual sync + throttled auto-sync when online; re-imported schedules reconcile-and-flag, surrogates marked, diff-to-confirm.
...
- ~~**Topic 18:** Q18.1 – Q18.5~~ ✓ **CLOSED 2026-09-01** — confirmed in §18.1. Two Vercel projects, dev/prod Supabase, `docs/ops/SETUP.md`, `.env.example`, mini CI and the `develop`/`main` model (2026-08-31, Q18.2); then production/preview/local with **preview on the dev Supabase project**, migrations by CLI from `packages/db`, **no custom domain** (Q18.1), **Sentry errors only — no PII, no tracing, no analytics** (Q18.3), **no self-hosted / venue laptop** (Q18.4), **team-owned accounts + annual handover checklist** (Q18.5).
...
All seven top blockers are now closed, and **topics 1–15 plus 18 are fully CLOSED** — every feature topic, the system architecture, and deployment/ops. Remaining: **19** (delivery phases) and the partials **16 / 17 / 20**. The next work is **build-shaped, not feature-shaped**: close 16, 17 and 19, then write `SPEC-FINAL.md` and `IMPLEMENTATION-PLAN.md`.
...
|---|---|---|---|
| v0.26 | 2026-09-01 | 0, 0.2, 14 (rewritten & closed), 18 (closed), 21, 22, 23, 24 | **Topic 18 CLOSED** and **topic 14 CLOSED as deferred-in-full.** §18.2's proposals promoted into §18.1 (production/preview/local, **preview on the dev Supabase project**, Turborepo caching, migrations by CLI from `packages/db`, client/server env-var split) and Q18.1/Q18.3–Q18.5 decided by Claude: no custom domain in v1 (short `*.vercel.app`); **Sentry free tier for errors only** — no PII, no tracing/replay, lazily loaded, offline-queued, no usage analytics; **no self-hosted or venue-laptop deployment**; **all accounts team-owned** with an inventory and annual handover checklist in `docs/ops/SETUP.md`; plus a pre-event operational checklist there. §14 rewritten as CLOSED/deferred with a "recorded design" subsection: Q14.2–Q14.5 answered (key at build time, results imported, manual + throttled auto-sync, **reconcile-and-flag on re-import** with surrogates marked and a diff to confirm), and the §24 row expanded to point at it. Status table, decision log, questions index and §24 updated. |
