...
**Status:** DRAFT v0.30 — **topics 1–16 and 18 CLOSED.** Every feature topic plus the system architecture and the deployment/operations model. Core forks decided (storage, versioning, offline stats, stack, access model, language, roles/auth, data-entry UX, offline-first & sync, realtime deferred → 45s refresh, dashboards & visualisation, search/ranking/browse, data quality, alliance selection, repo layout & services, environments & operations). Topic 14 is closed as **deferred in full to §24**. Remaining: **19**, and the partials **17** and **20**.
...
- ~~**Q18.6** **[RAISED BY ME]** — Off-platform backup/export before every event?~~ ✓ **CLOSED 2026-09-01: → §24 nice-to-have.**
...
**Closed 2026-09-01:** Q18.1, Q18.3–Q18.11 → confirmed requirements or §24 (**topic 18 CLOSED**). Q16.1, Q16.3–Q16.12 → confirmed requirements or §24 (**topic 16 CLOSED**), and §16.2's proposals promoted. Q14.2–Q14.5 answered and recorded as the deferred design (**topic 14 CLOSED — deferred in full to §24**).
...
All seven top blockers are now closed, and **topics 1–16 plus 18 are fully CLOSED** — every feature topic, the system architecture, the UI/UX design system and the deployment/operations model. Remaining: **19** (delivery phases) and the partials **17** (non-functional targets) and **20** (AI/MCP setup). The next work is **build-shaped, not feature-shaped**: close 17 and 19, then write `SPEC-FINAL.md` and `IMPLEMENTATION-PLAN.md`.
...
| **Off-platform backup / season export** — one file holding a season's raw entries, forms, versions and pick lists, downloadable by an admin and copied automatically to storage outside Supabase | Q18.6, topic 18 | Wanted, not now. `COLLABORATION.md` §8 rule 7 asks for an export before every event, and the free tier has no point-in-time recovery, so until this exists **a lost Supabase project is a lost season**. Cheapest version when built: one `exportSeason` use case serving both a manual download and a scheduled job. **Gate: this must exist before the first real competition** — deferred for development only, not for an event. *(confirmed 2026-09-01)* |
