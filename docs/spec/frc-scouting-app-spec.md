...
| 4 | Seasons, competitions & events | PARTIAL — Q4.3 closed (flat events) |
...
| Scoring model | Per-field point configuration stored **with the form**: boolean → points if true, counter/number → points per unit, single/multi-select → points per selected option, plus a per-field **success definition** for success-rate metrics. Game-specific, **not backfillable**; drives the metric engine (topics 3 & 9). Mechanics: Q2.8. **[RAISED BY ME → confirmed]** |
...
- **Season scoring model** (Q2.1, Q2.8). Points are configured **per field, stored with the form**: a boolean scores its points when true, a counter/number scores points per unit, a single/multi-select scores points per selected option; each scorable field also carries a success definition. Captured at field-creation time (same non-backfillable gotcha as semantic metadata, §3.3). Remaining mechanics (phase, negatives, versioning, totals) are Q2.8 → topics 3 & 9.
...
**Q2.8 — scoring model.** ✓ **Confirmed:** points live on the field, stored with the form (boolean → points-if-true; counter/number → points-per-unit; single/multi-select → points-per-selected-option; plus a per-field success flag). These sub-questions are spun out to **topics 3 (field config) & 9 (metric engine)** and do not block closure:
- **Q2.8a** — Phase-dependent points (auto vs teleop): separate per-phase fields, or one field with per-phase values?
- **Q2.8b** — Negative points allowed (fouls/penalties), or are penalties tracked but never auto-summed?
- **Q2.8c** — Is "mission success" just points > 0, or a distinct per-field predicate (attempted vs succeeded) for success-rate metrics?
- **Q2.8d** — Does changing a field's points create a new form version, or edit in place like label/range (§3.1)? Retroactive or new-entries-only?
- **Q2.8e** — Is the scouted match/team score simply the sum of per-field points across robots (no per-alliance data, Q2.4), and is that what's reconciled against the reserved official result (Q2.5)?
- **Q2.8f** — Alliance-level bonus/threshold points (ranking-point thresholds, coopertition) — out of scope for the scouted score? (Recommend yes, out of scope.)
...
**Scoring attributes (from topic 2).** Fields that contribute to the game score also carry point value(s) — per enum option where relevant — and an optional success predicate, captured here at creation time for the same non-backfillable reason. This is the **season scoring model** (§2.1). Exact storage and formula mechanics are Q2.8, resolved with the metric engine in topic 9.
...
| 2026-08-17 | 2 | **Topic 2 CLOSED.** Fixed/flexible split confirmed; Event = name+year, Team = number+name. Added a **per-field scoring model** (points stored with the form; per-enum-option points + per-field success criteria) captured at field-creation time. v1 form kinds = match + super only. Scout all 6 robots incl. our own. No per-alliance data. | Fixes the stable-vs-game-specific boundary the whole app rests on. The scoring model is game-specific yet can't live loose in a payload — metrics need it structured, and like semantic metadata it can't be backfilled, so it's captured up front. Match+super match the team's real workflow; pit/other stay cheap to add via the `kind` field. |
| 2026-08-17 | 2 | **Official results reserved-nullable; practice/playoff stored but excluded from metrics; own robot is a regular robot; scoring configured per-field on the form.** | Reserving the results schema now avoids a later migration and costs nothing if TBA import never lands — paired with a UI rule to hide absent scores, never show blanks. Practice/playoff data is useful to browse but would distort averages, so only qualification matches feed metrics. Our robot is just one of the 6 scouted robots — no separate tracking module to build. Storing points on each field keeps the scoring model versioned with the form and drives the metric engine directly. |
...
**Closed 2026-08-17:** Q2.1 – Q2.7 → confirmed requirements (**topic 2 CLOSED**); Q2.9 closed (own robot = regular robot); Q4.3 closed (flat events, no division hierarchy). New spun-out: Q2.8a – Q2.8f → topics 3 & 9.

- ~~**Topic 1:** Q1.1 – Q1.6~~ ✓ **CLOSED** — all confirmed in §1.1.
- ~~**Topic 2:** Q2.1 – Q2.7~~ ✓ **CLOSED** — confirmed in §2. Own robot = regular robot (Q2.9 closed). Spun out: Q2.8a–Q2.8f (scoring mechanics) → topics 3 & 9.
- **Topic 3:** Q3.1 – Q3.10
- **Topic 4:** Q4.1, Q4.2, Q4.4, Q4.5 — ~~Q4.3~~ ✓ CLOSED 2026-08-17 (flat events, no division hierarchy)
- **Topic 5:** Q5.1 – Q5.8
- **Topic 6:** Q6.1 – Q6.8
- **Topic 7:** Q7.1 – Q7.7
- **Topic 8:** Q8.1 – Q8.4
...
All seven top blockers are now closed. Next-tier durable questions still open: Q3.7 (delete-form semantics), Q9.4 (duplicate-scout resolution), Q6.5 (dead-robot status), Q5.2 (scouter login), Q2.8a–Q2.8f (scoring-model mechanics → topics 3/9).
